...
TEST(tets_epass_verification_and_parsing, test_verify_real_passport_certificate_with_masterlist)
{
  std::ifstream myfile2("validCert.cer", std::ios::binary);
  ByteVector tmp2;
  std::string stock2;
  std::shared_ptr<logicalaccess::EPassIdentityCardService> srv = std::make_shared<logicalaccess::EPassIdentityCardService>(nullptr);
  ...
  ASSERT_EQ(1, srv->verifyCertificate(tmp2, tmp));
}
...
TEST(tets_epass_verification_and_parsing, test_verify_real_passport_certificate)
{
  std::ifstream myfile2("validCert.cer", std::ios::binary);
  ByteVector tmp2;
  std::string stock2;
